      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        ...
        <div>
          ...
          <div className="max-w-sm">
            <p className="text-[9px] lg:text-[10px] text-slate-600 border-b border-slate-300 pb-0.5 mb-2 tracking-wide">
                ...
                {showDropdown && (
                  <div className="absolute z-20 w-full mt-1 bg-white border rounded-sm shadow-lg max-h-48 overflow-y-auto">
                    {suggestions.length === 0 ? (
                      <div className="px-3 py-2 text-sm text-slate-600">Keine Kunden gefunden</div>
                    ) : suggestions.map(c => {
                      ...
                      return (
                        <button key={c.id} onClick={() => { handleCustomerChange(c.id); setKundeSearch(""); setShowDropdown(false); }}
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {c.firma && <span className="text-slate-600 ml-1">({c.firma})</span>}
                            {c.email && <span className="text-xs text-slate-600 block truncate">{c.email}</span>}
                          </div>
