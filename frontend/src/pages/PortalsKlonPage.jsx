...
const PortalsKlonPage = () => {
  ...
  const loadPortals = useCallback(async () => {
    try {
      const res = await api.get("/portals-klon");
      setPortals(res.data);
    } catch (e) {
  ...
  const loadFiles = async (portalId) => {
    try {
      const res = await api.get(`/portals-klon/${portalId}/files`);
  ...
  const toggleActive = async (portal) => {
    try {
      await api.put(`/portals-klon/${portal.id}`, { active: !portal.active });
  ...
  const deletePortal = async (portalId) => {
    ...
    try {
      await api.delete(`/portals-klon/${portalId}`);
  ...
  const uploadBusinessFile = async (e) => {
    ...
    try {
      await api.post(`/portals-klon/${selectedPortal.id}/upload`, formData, {
  ...
  const deleteFile = async (fileId) => {
    try {
      await api.delete(`/portals-klon/files/${fileId}`);
...
const CreatePortalDialog = ({ customers, onClose, onCreated }) => {
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    try {
      const res = await api.post("/portals-klon", {
      ...
      if (sendEmail && customerEmail) {
        try {
          await api.post(`/portals-klon/${res.data.id}/send-email`, { portal_url: portalUrl });
...
const PortalDetail = ({ portal, files, onBack, onUpload, onDeleteFile, onToggle, onCopyLink, getPortalUrl }) => {
  ...
  useEffect(() => {
    api.get(`/portals-klon/${portal.id}/files`).catch(() => {});
    api.get("/portals-klon").then(res => {
    ...
    api.post(`/portals-klon/${portal.id}/mark-read`, {}).catch(() => {});
  }, [portal.id]);
  ...
  const sendAdminNote = async () => {
    ...
    try {
      ...
      if (newEmail !== origEmail) {
        ...
        await api.put(`/portals-klon/${portal.id}`, { customer_email: newEmail });
        portal.customer_email = newEmail; // lokal updaten
        emailUpdated = true;
      }
      const note = await api.post(`/portals-klon/${portal.id}/admin-notes`, { text: msgText });
