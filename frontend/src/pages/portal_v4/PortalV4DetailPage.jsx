...
/**
 * Portal v4 – Admin-Detail: Chat + Uploads für einen Account.
 */
export function PortalV4DetailPage() {
  const { id } = useParams();
  ...
  const loadAccount = async () => {
    try {
      const res = await api.get(`/portal-v4/admin/accounts/${id}`);
      setAccount(res.data);
    } catch (err) {
  ...
  const loadMessages = async () => {
    try {
      const res = await api.get(`/portal-v4/admin/accounts/${id}/messages`);
  ...
  const loadUploads = async () => {
    try {
      const res = await api.get(`/portal-v4/admin/accounts/${id}/uploads`);
  ...
  const sendMessage = async () => {
    ...
    if (!text) return;
    try {
      await api.post(`/portal-v4/admin/accounts/${id}/messages`, { text });
      setNewMsg("");
      loadMessages();
    } catch (err) {
      toast.error(err?.response?.data?.detail || err.message);
  ...
  const uploadFile = async (file) => {
    ...
    try {
      ...
      await api.post(`/portal-v4/admin/accounts/${id}/uploads`, fd, {
  ...
  const deleteUpload = async (up) => {
    ...
    try {
      await api.delete(`/portal-v4/admin/accounts/${id}/uploads/${up.id}`);
  ...
  const openFile = async (up) => {
    try {
      const res = await api.get(
        `/portal-v4/admin/accounts/${id}/uploads/${up.id}/file`,
        { responseType: "blob" }
      );
  ...
  const AdminThumb = ({ upload }) => {
    ...
    useEffect(() => {
      ...
      const run = async () => {
        try {
          const res = await api.get(
            `/portal-v4/admin/accounts/${id}/uploads/${upload.id}/file`,
            { responseType: "blob" }
          );
          if (cancel) return;
...
export default PortalV4DetailPage;
