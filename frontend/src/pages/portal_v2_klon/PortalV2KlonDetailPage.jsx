...
/**
 * Portal (Klon) – Admin-Detail: Chat + Uploads für einen Account.
 */
export function PortalV2KlonDetailPage() {
  const { id } = useParams();
  ...
  const loadAccount = async () => {
    try {
      const res = await api.get(`/portal-v2-klon/admin/accounts/${id}`);
      setAccount(res.data);
    } catch (err) {
  ...
  const loadMessages = async () => {
    try {
      const res = await api.get(`/portal-v2-klon/admin/accounts/${id}/messages`);
  ...
  const loadUploads = async () => {
    try {
      const res = await api.get(`/portal-v2-klon/admin/accounts/${id}/uploads`);
  ...
  const sendMessage = async () => {
    ...
    if (!text) return;
    try {
      await api.post(`/portal-v2-klon/admin/accounts/${id}/messages`, { text });
      setNewMsg("");
      loadMessages();
    } catch (err) {
      toast.error(err?.response?.data?.detail || err.message);
  ...
  const uploadFile = async (file) => {
    ...
    try {
      ...
      await api.post(`/portal-v2-klon/admin/accounts/${id}/uploads`, fd, {
  ...
  const deleteUpload = async (up) => {
    ...
    try {
      await api.delete(`/portal-v2-klon/admin/accounts/${id}/uploads/${up.id}`);
  ...
  const openFile = async (up) => {
    try {
      const res = await api.get(
        `/portal-v2-klon/admin/accounts/${id}/uploads/${up.id}/file`,
        { responseType: "blob" }
      );
  ...
  const AdminThumb = ({ upload }) => {
    ...
    useEffect(() => {
      ...
      const run = async () => {
        try {
          const res = await api.get(
            `/portal-v2-klon/admin/accounts/${id}/uploads/${upload.id}/file`,
            { responseType: "blob" }
          );
          if (cancel) return;
  ...
  return (
    <div className="space-y-4" data-testid="portal-v2-detail-page">
      <div className="flex items-center gap-3">
        <button
          onClick={() => navigate("/portal-v2")}
          className="p-2 rounded-lg border hover:bg-muted"
          data-testid="portal-v2-detail-back"
        >
      ...
      <div className="border rounded-xl bg-card overflow-hidden">
        <div className="border-b flex">
          <button
            ...
            data-testid="portal-v2-tab-messages"
          >
          ...
          <button
            ...
            data-testid="portal-v2-tab-uploads"
          >
          ...
          <div className="flex flex-col" style={{ height: "60vh" }}>
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#fafaf8]">
              ...
              {messages.map((m) => {
                ...
                return (
                  <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`} data-testid={`portal-v2-msg-${m.id}`}>
            ...
            <div className="p-3 border-t bg-white flex gap-2">
              <textarea
                ...
                data-testid="portal-v2-msg-input"
              />
              <button
                onClick={sendMessage}
                ...
                data-testid="portal-v2-msg-send"
              >
            ...
            {uploads.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {uploads.map((up) => {
                  ...
                  return (
                    <div key={up.id} className="border rounded-lg overflow-hidden bg-white group relative" data-testid={`portal-v2-upload-${up.id}`}>
                      <button
                        onClick={() => openFile(up)}
...
export default PortalV2KlonDetailPage;
