 ...
 * Backend: /api/portal-v2-klon/admin/*
 */
export function PortalV2KlonAdminPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [sRes, aRes] = await Promise.all([
        api.get("/portal-v2-klon/admin/settings"),
        api.get("/portal-v2-klon/admin/accounts"),
      ]);
  ...
  const toggleFeature = async () => {
    try {
      const res = await api.put("/portal-v2-klon/admin/settings", { feature_enabled: !settings?.feature_enabled });
  ...
  const save = async () => {
    ...
    try {
      if (editAccount) {
        await api.put(`/portal-v2-klon/admin/accounts/${editAccount.id}`, form);
        toast.success("Account aktualisiert");
      } else {
        await api.post("/portal-v2-klon/admin/accounts", form);
  ...
  const removeAccount = async (account) => {
    ...
    try {
      await api.delete(`/portal-v2-klon/admin/accounts/${account.id}`);
  ...
  const sendInvite = async (account) => {
    ...
    try {
      const res = await api.post(`/portal-v2-klon/admin/accounts/${account.id}/invite`);
      if (res.data?.sent) {
    ...
    <div className="space-y-6" data-testid="portal-v2-admin-page">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">Kundenportal (Klon)</h1>
          ...
          <div className="divide-y">
            {accounts.map((a) => (
              <div
                ...
                onClick={() => navigate(`/portal-v2-klon/detail/${a.id}`)}
...
export default PortalV2KlonAdminPage;
