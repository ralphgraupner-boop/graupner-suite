...
/**
 * Portal (Klon) – Import aus module_kunden
 * Modal mit Kundenliste, Suche, Status-Filter, Bulk-Select.
 */
export function PortalV2KlonImportDialog({ open, onClose, onImported }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await api.get("/portal-v2-klon/admin/kunden-quelle", { params });
  ...
  const doImport = async () => {
    ...
    setImporting(true);
    try {
      const res = await api.post("/portal-v2-klon/admin/accounts/import-from-kunden", {
...
export default PortalV2KlonImportDialog;
