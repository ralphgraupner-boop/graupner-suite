...
import { PortalV3ImportDialog } from "@/pages/portal_v3/PortalV3ImportDialog";

/**
 * Kundenportal v3 – Admin-Übersicht (Phase 1: Gerüst + CRUD)
 *
 * Strikt isoliert. Keine Abhängigkeit zum alten PortalsPage / portal.py.
 * Backend: /api/portal-v3/admin/*
 */
export function PortalV3AdminPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [sRes, aRes] = await Promise.all([
        api.get("/portal-v3/admin/settings"),
        api.get("/portal-v3/admin/accounts"),
      ]);
  ...
  const toggleFeature = async () => {
    try {
      const res = await api.put("/portal-v3/admin/settings", { feature_enabled: !settings?.feature_enabled });
  ...
  const save = async () => {
    ...
    try {
      if (editAccount) {
        await api.put(`/portal-v3/admin/accounts/${editAccount.id}`, form);
        toast.success("Account aktualisiert");
      } else {
        await api.post("/portal-v3/admin/accounts", form);
  ...
  const removeAccount = async (account) => {
    ...
    try {
      await api.delete(`/portal-v3/admin/accounts/${account.id}`);
  ...
  const sendInvite = async (account) => {
    ...
    try {
      const res = await api.post(`/portal-v3/admin/accounts/${account.id}/invite`);
      if (res.data?.sent) {
  ...
  return (
    <div className="space-y-6" data-testid="portal-v3-admin-page">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">Kundenportal v3</h1>
        ...
        <div className="flex gap-2">
          <button
            ...
            data-testid="portal-v3-settings-btn"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border hover:bg-muted text-sm"
            data-testid="portal-v3-import-btn"
          >
          ...
          <button
            ...
            data-testid="portal-v3-new-account-btn"
          >
      ...
      {showSettings && settings && (
        <div className="p-4 border rounded-xl bg-card space-y-3" data-testid="portal-v3-settings-panel">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              data-testid="portal-v3-feature-toggle"
            >
              {settings.feature_enabled ? "Aktiv" : "Inaktiv"}
            </button>
          </div>
      ...
      {/* Accounts-Tabelle */}
      <div className="border rounded-xl overflow-hidden bg-card" data-testid="portal-v3-accounts-list">
          ...
          <div className="divide-y">
            {accounts.map((a) => (
              <div
                ...
                onClick={() => navigate(`/portal-v3/detail/${a.id}`)}
                data-testid={`portal-v3-account-row-${a.id}`}
              >
                ...
                <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                  ...
                  <button
                    ...
                    data-testid={`portal-v3-invite-${a.id}`}
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground"
                    title="Bearbeiten"
                    data-testid={`portal-v3-edit-${a.id}`}
                  >
                  ...
                  <button
                    ...
                    title="Löschen"
                    data-testid={`portal-v3-delete-${a.id}`}
                  >
      ...
      {showNew && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowNew(false)}>
          <div
            ...
            data-testid="portal-v3-account-dialog"
          >
            ...
            <div className="space-y-3">
              <label className="block">
                ...
                <input
                  ...
                  data-testid="portal-v3-form-name"
                />
              </label>
              <label className="block">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border rounded-lg text-sm"
                  data-testid="portal-v3-form-email"
                />
              </label>
              <label className="block">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border rounded-lg text-sm"
                  rows={3}
                  data-testid="portal-v3-form-notes"
                />
              </label>
            </div>
            <div className="flex justify-end gap-2 pt-2 border-t">
              <button
                ...
                data-testid="portal-v3-form-cancel"
              >
                Abbrechen
              </button>
              <button
                ...
                data-testid="portal-v3-form-save"
              >
                Speichern
              </button>
            </div>
...
export default PortalV3AdminPage;
