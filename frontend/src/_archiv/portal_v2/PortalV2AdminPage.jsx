...
export function PortalV2AdminPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [sRes, aRes] = await Promise.all([
        api.get("/portal-v2/admin/settings"),
        api.get("/portal-v2/admin/accounts"),
      ]);
  ...
  const toggleFeature = async () => {
    try {
      const res = await api.put("/portal-v2/admin/settings", { feature_enabled: !settings?.feature_enabled });
  ...
  const save = async () => {
    ...
    try {
      if (editAccount) {
        await api.put(`/portal-v2/admin/accounts/${editAccount.id}`, form);
        toast.success("Account aktualisiert");
      } else {
        await api.post("/portal-v2/admin/accounts", form);
    ...
    <div className="space-y-6" data-testid="portal-v2-admin-page">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">Kundenportal v2</h1>
...
export default PortalV2AdminPage;
