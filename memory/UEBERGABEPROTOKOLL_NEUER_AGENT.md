...
16. **Hilfe: Betterbird-Helfer-Anleitung** fest in `HilfeTab.jsx` (Einstellungen → Hilfe, neuer Abschnitt nach E-Mail-Modul): Schritt-für-Schritt Installation des `windows-helfer/` (install.bat → C:\\Graupner → Protokoll bbcompose://), inkl. .eml-Fallback. Helfer-Dateien liegen unter `/app/windows-helfer/`. (23.06.2026)
OFFENE WÜNSCHE (geplant, noch nicht gebaut): (a) Status „Auftrag nicht erhalten" für Angebot+AB inkl. Ausblenden aus aktiven Listen (Regel 17) — Plan vorgelegt, a/b-Rückfrage offen. (b) Druck-/Versand-Dateiname mit sprechendem Namen statt langer Blob-Nummer (Frontend-Lösung, pdf_generator bleibt gesperrt) — Rückfrage Betreff ja/nein offen.
17. **Versand kinderleicht**: Mail-Dialog in `WysiwygDocumentEditor.jsx` vereinfacht (alten bb/.eml-Umschalter entfernt). Hauptknopf „E-Mail jetzt vorbereiten" (= zuverlässiger .eml-Weg, funktioniert immer). Freundlicher Hinweis „Betterbird öffnet sich nicht von selbst?" mit zwei Knöpfen: „Mach das für mich" (.eml) und „Zeig mir, wie ich es einrichte" (→ `/settings?tab=hilfe`). `SettingsPage.jsx` öffnet jetzt per `?tab=`-Parameter direkt den richtigen Tab. (23.06.2026)
18. **Betterbird-Erkennung + Helfer-Download**: `DocumentPreview.jsx` → `openBetterbird()` zeigt nach 2 Sek. einen Dialog „Hat sich Betterbird geöffnet? Ja / Nein". Bei „Nein" wird `windows-helfer/Graupner_Betterbird_Setup.bat` von GitHub-raw heruntergeladen (URL ralphgraupner-boop/graupner-suite/main) + Hinweis „per Doppelklick ausführen". NEU: `windows-helfer/Graupner_Betterbird_Setup.bat` — in sich geschlossen (eingebettetes bbcompose-Skript, findet Betterbird, registriert bbcompose:// pro Benutzer, keine externen Abhängigkeiten). Frontend kompiliert. OFFEN/UNGETESTET: .bat auf echtem Windows; ggf. raw-URL durch github.com/.../raw/ ersetzen, falls Browser die Datei als Text anzeigt statt herunterzuladen. (23.06.2026)
