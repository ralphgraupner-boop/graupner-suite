...
- **Mobile-Pass für Feedback-Widget + Kunden-Liste**: Floating-Notiz-Button auf 56×56 px mit Safe-Area-Inset, Form mit `enterKeyHint="send"` (echter Senden-Knopf auf Handy-Tastatur), Touch-Targets ≥44px, font-size 16px (kein iOS-Zoom). Kunden-Liste auf Handy: Name + Status prominent, Email/Telefon untereinander, Action-Icons (Projekte/Edit/Löschen) erst ab `sm:` (am Handy via Aufklapp-Detail erreichbar). Checkbox + Sammel-Export auch erst auf Tablet/Desktop.
