...
- **Mail-Detail & Mailverlauf-Modal**: Neuer Endpunkt `POST /mail-detail` (Body + Header einer konkreten IMAP-Mail) und `POST /customer-mails` (Suche in allen aktiven Postfächern nach From/To/CC = Kunden-Mail, ein/aus, max 30/letzte 26 Wochen). UI-Komponente `MailHistoryModal.jsx` zeigt Liste + Detail-Spalte. „Anzeigen"-Button in der Vorschau zeigt Mail-Inhalt vor Import. Neuer Aktion-Button **„Mailverlauf"** auf jeder Kunden-Detailkarte (gelb) öffnet das Modal — komplette Mail-Historie eines Kunden direkt in der Suite, kein Wechsel zu Betterbird nötig.
- **Anfrage-Statistik pro Postfach**: Neuer Endpunkt `GET /api/module-mail-inbox/stats?days=N` (Mongo-Aggregation auf `module_mail_inbox`). UI als klappbare Karte oben auf der Mail-Anfragen-Seite mit Zeitraum-Auswahl (7/30/90/365 Tage), Gesamt-Kacheln (Anfragen / Übernommen / Offen / Spam / **Conversion-Rate**) und Aufschlüsselung pro `account_label`.
