"""
Portal v2 – Sync/Import aus module_kunden (nur lesend)

Strikt isoliert:
- Liest aus module_kunden
- Schreibt NUR in portal2klon_accounts und portal2klon_sync_log
...
async def list_kunden_for_import(
    ...
    Nur lesend.
    Markiert, welche bereits im Portal v2 existieren.
    """
    query = {}
    if status:
    ...
    # Existenz-Check gegen portal2klon_accounts
    existing_emails = set()
    existing_customer_ids = set()
    async for acc in db.portal2klon_accounts.find({}, {"_id": 0, "email": 1, "customer_id": 1}):
        if acc.get("email"):
...
async def import_from_kunden(payload: ImportRequest, user=Depends(get_current_user)):
    """
    Bulk-Import ausgewählter Kunden-IDs aus module_kunden in portal2klon_accounts.
    Schreibt Audit-Log nach portal2klon_sync_log.
    """
    ...
    for kid in payload.kunden_ids:
        ...
        existing = await db.portal2klon_accounts.find_one({"$or": dup_query}, {"_id": 0, "id": 1})
        if existing:
    ...
    await db.portal2klon_sync_log.insert_one(log_entry)
    log_entry.pop("_id", None)

    logger.info(f"Portal v2 Import: {len(imported)} importiert, {len(skipped)} übersprungen")
    return {
        "imported": imported,
        "skipped": skipped,
...
async def list_sync_log(limit: int = 20, user=Depends(get_current_user)):
    ...
    logs = await db.portal2klon_sync_log.find({}, {"_id": 0}).sort("timestamp", -1).to_list(limit)
    return logs
