...
Alle Endpoints unter /api/portal-v2-klon/admin/*
...
SETTINGS_ID = "portal2klon_settings_default"


# ============== SETTINGS ==============

async def _get_settings_doc() -> dict:
    doc = await db.portal2klon_settings.find_one({"id": SETTINGS_ID}, {"_id": 0})
    if not doc:
        defaults = PortalV2Settings().model_dump()
        defaults["id"] = SETTINGS_ID
        await db.portal2klon_settings.insert_one(defaults)
...
async def update_settings(update: PortalV2SettingsUpdate, user=Depends(get_current_user)):
    ...
    await db.portal2klon_settings.update_one({"id": SETTINGS_ID}, {"$set": changes})
    new_doc = await db.portal2klon_settings.find_one({"id": SETTINGS_ID}, {"_id": 0})
    new_doc.pop("id", None)
...
async def list_accounts(user=Depends(get_current_user)):
    ...
    accounts = await db.portal2klon_accounts.find(
        {},
...
async def create_account(payload: PortalAccountCreate, user=Depends(get_current_user)):
    ...
    existing = await db.portal2klon_accounts.find_one({"email": email})
    if existing:
    ...
    await db.portal2klon_accounts.insert_one(account)
...
async def get_account(account_id: str, user=Depends(get_current_user)):
    account = await db.portal2klon_accounts.find_one(
        {"id": account_id},
        {"_id": 0, "password_hash": 0, "token": 0},
    )
    if not account:
...
async def update_account(account_id: str, update: PortalAccountUpdate, user=Depends(get_current_user)):
    existing = await db.portal2klon_accounts.find_one({"id": account_id})
    if not existing:
        raise HTTPException(404, "Account nicht gefunden")
    ...
    if "email" in changes:
        ...
        if changes["email"] != existing.get("email"):
            dup = await db.portal2klon_accounts.find_one({"email": changes["email"], "id": {"$ne": account_id}})
            if dup:
    ...
    if not changes:
        ...
        return existing
    changes["updated_at"] = datetime.now(timezone.utc).isoformat()
    await db.portal2klon_accounts.update_one({"id": account_id}, {"$set": changes})
    updated = await db.portal2klon_accounts.find_one(
        {"id": account_id},
        {"_id": 0, "password_hash": 0, "token": 0},
    )
    return updated
...
async def delete_account(account_id: str, user=Depends(get_current_user)):
    result = await db.portal2klon_accounts.delete_one({"id": account_id})
...
async def _prepare_invite(account_id: str, request: Request | None = None) -> tuple[dict, str, str]:
    ...
    account = await db.portal2klon_accounts.find_one({"id": account_id}, {"_id": 0})
    if not account:
        raise HTTPException(404, "Account nicht gefunden")
    ...
    await db.portal2klon_accounts.update_one(
        {"id": account_id},
        {"$set": {
    ...
    login_url = f"{frontend_base.rstrip('/')}/portal-v2-klon/login/{token}"
...
async def send_invite(account_id: str, request: Request, user=Depends(get_current_user)):
    ...
    await db.portal2klon_activity.insert_one({
