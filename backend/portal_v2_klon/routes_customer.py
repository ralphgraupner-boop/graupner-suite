"""
Portal v2 – Kunden-Routes (öffentlich / Portal-Token-auth).
...
async def login(body: LoginBody):
    ...
    account = await db.portal2klon_accounts.find_one(query, {"_id": 0})
    if not account or not account.get("password_hash"):
    ...
    logger.info(f"Portal v2 Login: {account['email']}")
    return {
        "session": session,
        "account": {
            "id": account["id"],
