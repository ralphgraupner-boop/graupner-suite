...
async def send_admin_notification_email(name, anrede, form_dict, address, topics, obj_addr, photo_attachments):
    ...
    try:
        ...
        if photo_attachments:
            ...
            <div style="background:#f0f7ff;border-left:4px solid #2563eb;padding:16px;margin:20px 0;border-radius:8px">
                <h3 style="color:#2563eb;font-size:15px;margin:0 0 8px 0">📷 {len(photo_attachments)} Bild(er) hochgeladen</h3>
        ...
        send_email(
            ...
            body_html=body_html,
            attachments=photo_attachments if photo_attachments else None
        )
        logger.info(f"✅ Admin-Benachrichtigung an {admin_email} gesendet (mit {len(photo_attachments)} Anhängen)" if photo_attachments else f"✅ Admin-Benachrichtigung an {admin_email} gesendet")
...
async def kontakt_relay(request: Request):
    ...
    try:
        ...
        import asyncio
        asyncio.create_task(send_admin_notification_email(
            name=name,
            anrede=anrede,
            form_dict=form_dict,
            address=address,
