...
async def fetch_imap_to_inbox(creds: dict) -> int:
    ...
    for eid in email_ids[-1000:]:
        ...
        # Groessen-Schutz (03.07.2026, Ralph): MongoDB erlaubt max. ~16MB pro
        # Dokument. Anhaenge (z.B. hochaufgeloeste Fotos) koennen das nach
        # Base64-Kodierung leicht sprengen und haben bisher den GESAMTEN
        # Abruf-Durchlauf abgebrochen (keine Fehlerbehandlung). Jetzt: zu
        # grosse Anhaenge werden uebersprungen (nur Metadaten gespeichert),
        # und ein Fehler bei einem einzelnen Anhang bricht nicht mehr den
        # kompletten Durchlauf ab.
        MAX_ATTACHMENT_BYTES = 10_000_000  # 10 MB Sicherheitsgrenze
        raw_attachments = get_attachments(msg)
        attachment_meta = []
        for att in raw_attachments:
            att_id = str(uuid.uuid4())
            too_large = (att.get("size") or 0) > MAX_ATTACHMENT_BYTES
            try:
                if not too_large:
                    ...
                    logger.warning(f"IMAP: Anhang '{att['filename']}' ({att.get('size')} bytes) zu gross - nicht gespeichert, nur Metadaten")
            except Exception as att_err:
                logger.error(f"IMAP: Anhang '{att.get('filename')}' konnte nicht gespeichert werden: {att_err}")
                too_large = True
        ...
        # Auto-Import ins Kontakt-Modul DEAKTIVIERT (03.07.2026, Ralph):
        # Hat unbemerkt fehlerhafte Kunden angelegt (Spam, eigene Adresse,
        # Lieferanten-Mails). Neue Anfragen sollen ausschliesslich ueber die
        # geprueften module_mail_inbox-Warteliste angelegt werden (Als Kunde
        # uebernehmen / Zuordnen), nie automatisch im Hintergrund.
        if False and classification == "anfrage":
            try:
                import re as _re
