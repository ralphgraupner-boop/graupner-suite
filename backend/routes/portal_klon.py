...
async def verify_portal(token: str, body: dict):
    portal = await db.portals_klon.find_one({"token": token}, {"_id": 0})
    if not portal:
        raise HTTPException(404, "Portal nicht gefunden")
...
async def public_upload_file(
    ...
    total_count = await db.portal_klon_files.count_documents({
        "portal_id": portal["id"],
        "is_deleted": False,
        "uploaded_by": "customer",
    })
    ...
    storage_path = f"graupner-suite/portals-klon/{portal['id']}/{uuid.uuid4()}.{ext}"
