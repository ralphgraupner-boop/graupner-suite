"""
Portal v3 – Sync/Import aus module_kunden (nur lesend)

Strikt isoliert:
- Liest aus module_kunden
- Schreibt NUR in portal3_accounts und portal3_sync_log
...
async def list_kunden_for_import(
    ...
    Nur lesend.
    Markiert, welche bereits im Portal v3 existieren.
    """
    query = {}
    if status:
    ...
    # Existenz-Check gegen portal3_accounts
    existing_emails = set()
    existing_customer_ids = set()
    async for acc in db.portal3_accounts.find({}, {"_id": 0, "email": 1, "customer_id": 1}):
        if acc.get("email"):
...
async def import_from_kunden(payload: ImportRequest, user=Depends(get_current_user)):
    """
    Bulk-Import ausgewählter Kunden-IDs aus module_kunden in portal3_accounts.
    Schreibt Audit-Log nach portal3_sync_log.
    """
    ...
    for kid in payload.kunden_ids:
        ...
        existing = await db.portal3_accounts.find_one({"$or": dup_query}, {"_id": 0, "id": 1})
        if existing:
    ...
    await db.portal3_sync_log.insert_one(log_entry)
    log_entry.pop("_id", None)

    logger.info(f"Portal v3 Import: {len(imported)} importiert, {len(skipped)} übersprungen")
    return {
        "imported": imported,
        "skipped": skipped,
...
async def list_sync_log(limit: int = 20, user=Depends(get_current_user)):
    ...
    logs = await db.portal3_sync_log.find({}, {"_id": 0}).sort("timestamp", -1).to_list(limit)
    return logs
