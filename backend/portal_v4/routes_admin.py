"""
Portal v4 – Admin-Routes
Alle Endpoints unter /api/portal-v4/admin/*
...
SETTINGS_ID = "portal4_settings_default"


# ============== SETTINGS ==============

async def _get_settings_doc() -> dict:
    doc = await db.portal4_settings.find_one({"id": SETTINGS_ID}, {"_id": 0})
    if not doc:
        defaults = PortalV4Settings().model_dump()
        defaults["id"] = SETTINGS_ID
        await db.portal4_settings.insert_one(defaults)
...
@router.put("/settings")
async def update_settings(update: PortalV4SettingsUpdate, user=Depends(get_current_user)):
    ...
    await db.portal4_settings.update_one({"id": SETTINGS_ID}, {"$set": changes})
    new_doc = await db.portal4_settings.find_one({"id": SETTINGS_ID}, {"_id": 0})
    new_doc.pop("id", None)
    logger.info(f"Portal v4 Settings aktualisiert: {list(changes.keys())}")
    return new_doc
...
async def list_accounts(user=Depends(get_current_user)):
    ...
    accounts = await db.portal4_accounts.find(
        {},
...
async def create_account(payload: PortalAccountCreate, user=Depends(get_current_user)):
    ...
    existing = await db.portal4_accounts.find_one({"email": email})
    if existing:
    ...
    logger.info(f"Portal v4 Account angelegt: {account['name']} ({account['email']})")
    return account
...
async def update_account(account_id: str, update: PortalAccountUpdate, user=Depends(get_current_user)):
    existing = await db.portal4_accounts.find_one({"id": account_id})
    if not existing:
        raise HTTPException(404, "Account nicht gefunden")
    ...
    if "email" in changes:
        ...
        if changes["email"] != existing.get("email"):
            dup = await db.portal4_accounts.find_one({"email": changes["email"], "id": {"$ne": account_id}})
            if dup:
    ...
    if not changes:
        ...
        return existing
    changes["updated_at"] = datetime.now(timezone.utc).isoformat()
    await db.portal4_accounts.update_one({"id": account_id}, {"$set": changes})
    updated = await db.portal4_accounts.find_one(
        {"id": account_id},
        {"_id": 0, "password_hash": 0, "token": 0},
    )
    return updated
...
async def delete_account(account_id: str, user=Depends(get_current_user)):
    result = await db.portal4_accounts.delete_one({"id": account_id})
    if result.deleted_count == 0:
        raise HTTPException(404, "Account nicht gefunden")
    logger.info(f"Portal v4 Account geloescht: {account_id}")
...
async def _prepare_invite(account_id: str, request: Request | None = None) -> tuple[dict, str, str]:
    ...
    login_url = f"{frontend_base.rstrip('/')}/portal-v4/login/{token}"
...
async def send_invite(account_id: str, request: Request, user=Depends(get_current_user)):
    ...
    logger.info(f"Portal v4 Einladung {'gesendet' if ok else 'fehlgeschlagen'}: {account['email']}")
    return {
        "sent": bool(ok),
        "email": account["email"],
        "login_url": login_url,
