...
def _convert_image(data: bytes, content_type: str, filename: str) -> tuple[bytes, str, str]:
    ...
    try:
        ...
        logger.warning(f"Portal v4 image convert failed, using raw: {e}")
...
async def _check_rate_limit(account_id: str) -> bool:
    ...
    settings = await db.portal4_settings.find_one({"id": "portal4_settings_default"}, {"_id": 0})
    if not settings:
        return False
    ...
    count = await db.portal4_uploads.count_documents({
...
async def _save_upload(account_id: str, file: UploadFile, description: str, uploaded_by: str) -> dict:
    ...
    await db.portal4_uploads.insert_one(doc)
    doc.pop("_id", None)
    return doc
...
async def customer_upload(
    ...
    logger.info(f"Portal v4 Upload von Kunde {account.get('email')}: {file.filename}")
    return doc


@router.get("/uploads")
async def customer_list_uploads(account=Depends(get_current_customer)):
    ups = await db.portal4_uploads.find(
...
async def customer_get_upload(upload_id: str, account=Depends(get_current_customer)):
    rec = await db.portal4_uploads.find_one(
...
async def admin_upload_for_account(
    ...
    logger.info(f"Portal v4 Admin-Upload für {account.get('email')}: {file.filename}")
    return doc
...
async def admin_delete_upload(account_id: str, upload_id: str, user=Depends(get_current_user)):
    res = await db.portal4_uploads.update_one(
