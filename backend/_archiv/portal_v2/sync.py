...
- Schreibt NUR in portal2_accounts und portal2_sync_log
...
async def list_kunden_for_import(
    ...
    # Existenz-Check gegen portal2_accounts
    existing_emails = set()
    existing_customer_ids = set()
    async for acc in db.portal2_accounts.find({}, {"_id": 0, "email": 1, "customer_id": 1}):
        if acc.get("email"):
...
async def import_from_kunden(payload: ImportRequest, user=Depends(get_current_user)):
    """
    Bulk-Import ausgewählter Kunden-IDs aus module_kunden in portal2_accounts.
    Schreibt Audit-Log nach portal2_sync_log.
    """
    ...
    for kid in payload.kunden_ids:
        ...
        existing = await db.portal2_accounts.find_one({"$or": dup_query}, {"_id": 0, "id": 1})
        if existing:
    ...
    await db.portal2_sync_log.insert_one(log_entry)
...
async def list_sync_log(limit: int = 20, user=Depends(get_current_user)):
    ...
    logs = await db.portal2_sync_log.find({}, {"_id": 0}).sort("timestamp", -1).to_list(limit)
    return logs
