"""
Portal v3 – Nachrichten (Chat Admin ↔ Kunde)
...
async def admin_send_message(account_id: str, body: MessageCreate, user=Depends(get_current_user)):
    ...
    await db.portal3_messages.insert_one(doc)
    doc.pop("_id", None)
    logger.info(f"Portal v3 Admin-Nachricht an {account.get('email')}")
    return doc
...
async def admin_unread_summary(user=Depends(get_current_user)):
    ...
    total = await db.portal3_messages.count_documents({
        "sender": "customer",
...
async def customer_send_message(body: MessageCreate, account=Depends(get_current_customer)):
    ...
    logger.info(f"Portal v3 Kunden-Nachricht von {account.get('email')}")
    return doc
