...
async def _build_portal_email_html(
    ...
    <div style="font-family:Arial,Helvetica,sans-serif;max-width:640px;margin:0 auto;color:#1f2937;line-height:1.55;">
      ...
      <h3 style="color:#1a5632;font-size:16px;margin:24px 0 8px 0;">📋 So funktioniert das Portal – Schritt für Schritt:</h3>
      <ol style="padding-left:20px;margin:0 0 16px 0;">
        ...
        <li style="margin-bottom:6px;">Laden Sie gewünschte <strong>Fotos hoch</strong> (z.&nbsp;B. vom Fenster, Türen, dem betroffenen Raum) – einfach per Klick oder Drag &amp; Drop.</li>
...
async def _check_rate_limit_or_lock(portal: dict) -> bool:
    ...
    recent = await db.portal_klon_files.count_documents({
        "portal_id": portal["id"],
        "is_deleted": False,
    ...
    if recent >= RATE_LIMIT_MAX_UPLOADS:
        # Auto-lock portal
        await db.portals_klon.update_one(
            {"id": portal["id"]},
...
@router.get("/portals-klon")
async def list_portals(user=Depends(get_current_user)):
    portals = await db.portals_klon.find({}, {"_id": 0}).sort("created_at", -1).to_list(200)
    return portals


@router.get("/portals-klon/lookup")
async def lookup_portal(email: str = "", customer_id: str = "", anfrage_id: str = "", user=Depends(get_current_user)):
    ...
    if not query["$or"]:
        return None
    portal = await db.portals_klon.find_one(query, {"_id": 0, "password_hash": 0})
    return portal


@router.post("/portals-klon/from-customer/{customer_id}")
async def create_portal_from_customer(customer_id: str, body: dict, user=Depends(get_current_user)):
    ...
    existing = await db.portals_klon.find_one({"customer_id": customer_id})
    if existing:
    ...
    await db.portals_klon.insert_one(portal)
    portal.pop("_id", None)
...
@router.post("/portals-klon/from-anfrage/{anfrage_id}")
async def create_portal_from_anfrage(anfrage_id: str, body: dict, user=Depends(get_current_user)):
    ...
    existing = await db.portals_klon.find_one({"anfrage_id": anfrage_id})
    if existing:
...
@router.put("/portals-klon/{portal_id}")
async def update_portal(portal_id: str, body: dict, user=Depends(get_current_user)):
    ...
    await db.portals_klon.update_one({"id": portal_id}, {"$set": updates})
    return {"message": "Aktualisiert"}


@router.delete("/portals-klon/{portal_id}")
async def delete_portal(portal_id: str, user=Depends(get_current_user)):
    await db.portals_klon.delete_one({"id": portal_id})
    await db.portal_klon_files.update_many({"portal_id": portal_id}, {"$set": {"is_deleted": True}})
    return {"message": "Gelöscht"}


@router.get("/portals-klon/{portal_id}/files")
async def admin_list_files(portal_id: str, user=Depends(get_current_user)):
    files = await db.portal_klon_files.find(
        {"portal_id": portal_id, "is_deleted": False}, {"_id": 0}
    ).sort("created_at", -1).to_list(500)
    return files


@router.post("/portals-klon/{portal_id}/upload")
async def admin_upload_file(
    ...
    portal = await db.portals_klon.find_one({"id": portal_id})
    if not portal:
    ...
    storage_path = f"graupner-suite/portals-klon/{portal_id}/{uuid.uuid4()}.{ext}"
    ...
    await db.portal_klon_files.insert_one(file_doc)
    file_doc.pop("_id", None)
    return file_doc


@router.delete("/portals-klon/files/{file_id}")
async def admin_delete_file(file_id: str, user=Depends(get_current_user)):
    await db.portal_klon_files.update_one({"id": file_id}, {"$set": {"is_deleted": True}})
    return {"message": "Datei gelöscht"}


@router.post("/portals-klon/{portal_id}/send-email")
async def send_portal_email(portal_id: str, body: dict, user=Depends(get_current_user)):
    portal = await db.portals_klon.find_one({"id": portal_id}, {"_id": 0})
    if not portal:
        raise HTTPException(404, "Portal nicht gefunden")
...
@router.post("/portals-klon/{portal_id}/admin-notes")
...
@router.get("/portals-klon/{portal_id}/admin-notes")
...
@router.get("/portals-klon/for-customer/{customer_id}")
async def portal_for_customer(customer_id: str, user=Depends(get_current_user)):
    ...
    portal = await db.portals_klon.find_one({"customer_id": customer_id}, {"_id": 0, "password_hash": 0})
    if not portal:
        return {"exists": False}
...
@router.post("/portals-klon/{portal_id}/mark-read")
...
@router.get("/portals-klon/unread-count")
